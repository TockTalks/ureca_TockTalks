...
| 목표 | 해결 기술 |
|---|---|
| 팀 전체가 외부 API 키 하나를 공유해도 안정적으로 개발 | Redis + Lua 스크립트 기반 분산 Rate Limiter (이후 개별 키 전환 + 로컬 페이싱으로 단순화) |
...
### 6-1. 팀 공유 Redis 최적화 → 지금은 로컬 페이싱으로 단순화
...
#### 처음엔 Redis + Lua 스크립트로 인스턴스 간 조율

여러 로컬 프로세스가 같은 키를 나눠 쓰는 문제를 풀기 위해, Redis에 "다음 호출 가능 시각"을 두고 Lua 스크립트로 원자적으로 조회+갱신하는 분산 Rate Limiter를 만들었습니다.
...
단순히 자바 코드에서 GET → 계산 → SET을 순차 실행했다면, 두 프로세스가 동시에 GET 해서 같은 옛날 값을 읽는 레이스 컨디션이 생겼을 겁니다. Lua 스크립트로 묶어야 여러 인스턴스가 동시에 호출해도 정확히 순서대로 시간표가 배정됩니다.

#### 근데 결국 개별 키로 전환하면서, Redis 조율 자체를 걷어냄

키 하나로는 팀 전체 테스트 트래픽을 감당하기 어려워 각자 개인 API 키를 발급받는 방식으로 전환했고, 배포도 단일 인스턴스라 애초에 "여러 프로세스가 조율할 상황" 자체가 사라졌습니다. 그래서 Redis 기반 조율(Lua 스크립트, appKey별 Redis 키, 장애 폴백)을 전부 제거하고, 이 인스턴스 하나가 스스로 속도를 맞추는 로컬(JVM) 페이싱만 남겼습니다.
...
단건 조회와 배치 조회가 모두 같은 `acquire()`를 거치므로, 보유 종목 수백 개를 배치(30개씩)로 조회하는 콜드캐시 상황에서도 이 페이싱 하나로 KIS 한도를 넘기지 않습니다.
...
- 이미 로테이션된 Refresh Token이 재사용되면 탈취 의심 신호로 보고 해당 회원의 Refresh/Access Token을 선제적으로 모두 무효화
- 이메일 기준 로그인 실패 잠금(5회 실패 시 5분, Redis 공유 카운터 — 인스턴스가 바뀌어도 방어 유지)
- 로그아웃/회원탈퇴 시 Access/Refresh Token 즉시 무효화
- JWT 서명 시크릿은 기본값 없이 `JWT_SECRET` 환경변수로만 주입 (아래 트러블슈팅 ④ 참고)
...
**해결**: 처음엔 Redis 기반 분산 Rate Limiter로 팀 전체의 초당 호출량을 조율했으나, 하나의 키로는 팀 전체 테스트 트래픽 자체를 감당하기 어려워 최종적으로는 각자 개별 API 키를 발급받는 방식으로 전환. 이후에는 배포도 단일 인스턴스라 인스턴스 간 조율 자체가 필요 없어져, Redis 조율 로직을 걷어내고 로컬(JVM) 페이싱만 남김 (6-1 참고)
...
### ③ 방 종료·참가 처리의 체크-후-저장 레이스 컨디션

**문제**: 방을 종료 처리하는 경로가 스케줄러(만료 방 자동 종료), 관리자 강제 종료, 유저 요청 시 지연(lazy) 체크까지 3곳이라 같은 방에 대해 동시에 여러 경로가 실행되면 랭킹 아카이브가 중복 저장될 수 있었고, 방 참가도 정원 확인과 저장 사이에 동시 요청이 겹치면 정원을 넘겨 참가될 수 있었음

**원인**: "상태 확인 → 저장"이 하나의 원자적 연산으로 묶여있지 않음

**해결**: 방(Room) 단위 비관적 락(`findByIdForUpdate`)을 도입해 방 종료(스케줄러/관리자/lazy 체크 3경로 전부)와 방 참가를 각각 "락 획득 → 상태 재확인 → 처리"로 직렬화. 락 획득 사이에 이미 처리된 경우를 재확인하는 테스트로 검증함

### ④ JWT 서명 시크릿이 플레이스홀더로 커밋됨

**문제**: `jwt.secret`이 `change-this-secret-key-before-deploy`라는 플레이스홀더 값 그대로 git에 커밋돼 있었음. 이 값만 있으면 누구나 임의 회원(관리자 포함)의 토큰을 위조할 수 있는 사실상의 인증 우회였음

**원인**: 개발 초기에 넣어둔 자리표시자 값을 실제로 채워 넣는 걸 놓치고 그대로 커밋함

**해결**: 기본값 없이 `${JWT_SECRET}` 환경변수를 필수로 요구하도록 변경 — 값을 안 채우면 기동 자체가 실패하게 해서, 플레이스홀더가 그대로 쓰일 가능성을 원천 차단함
...
- 발견된 이슈에 대한 실제 수정 (예: JWT 서명 시크릿 하드코딩 수정, Refresh Token 재사용 탐지 추가, KIS Rate Limiter의 불필요한 Redis 분산 조율 제거)
